...
export class LivreComponent implements OnInit {
  ...
  getallLivre() {
    this.usersService.getallLivre().then(d => {

      this.users = d.
        _embedded.livres.reverse()
  ...
  edit() {
    ...
    if (this.editUserForm.valid) {
      this.usersService.upadateLivre(this.editUserForm.controls.id.value, this.editUserForm.value).then(d => {
        this.getallLivre()
  ...
  delete(user,i) {
    ...
    let id = user._links.self.href.split('/').pop();
      ...
      this.usersService.deleteLivre(id).then(d => {
  ...
  add() {
    ...
    if (this.addUserForm.valid) {
      let id = 1
      this.usersService.addLivre(this.addUserForm.value).then(d => {
