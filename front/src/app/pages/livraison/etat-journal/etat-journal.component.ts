...
@Component({
  selector: 'app-etat-journal',
  templateUrl: './etat-journal.component.html',
  styleUrls: ['./etat-journal.component.scss']
})
export class EtatJournalComponent implements OnInit {
  money = 0
  nid
  ids1
  ids2
  ...
  selectCity(ev) {
    ...
    this.subDivisions = []
    this.selectedCity = ev.length ? ev.map(item => item.division) : this.sharedService.gouvernorats.map(item => item.division)
    for (let index = 0; index < this.selectedCity.length; index++) {
      for (let j = 0; j < this.sharedService.gouvernorats.length; j++) {
        if (this.selectedCity[index] == this.sharedService.gouvernorats[j].division) {
          this.subDivisions = this.subDivisions.concat(this.sharedService.gouvernorats[j].subDivisions)
  ...
  getHowMuchMoneyIOweIntigo() {
    this.money = 0
    this.manageDeliveryService.getHowMuchMoneyIOweIntigo(this.selectedClient).then(d => {
      this.money = d.total
      this.cdr.detectChanges()
